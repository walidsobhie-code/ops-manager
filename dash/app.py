...
# ── Inline analytics fallbacks (app works even if brain/analytics.py is broken) ──
...
def calculate_7day_baseline(store_reports):
...
def generate_fleet_summary_prompt(recent_reports):
    ...
    lines = [f"  - {r.get('store_id')}: sales={r.get('sales')}, "
             f"inv={r.get('inventory_status')}, staff={r.get('staffing')}, "
             f"note={r.get('analysis')}" for r in recent_reports]
    body = chr(10).join(lines)
    return (f"Analyse this retail fleet data and return ONLY valid JSON:\n{body}\n"
            'Schema: {"fleet_health_score":<0-100>,"strategic_recommendation":"<one sentence>",'
            '"critical_alerts":["<alert>"],"top_performer":"<store_id>","at_risk_stores":["<id>"]}')

def generate_intelligence_brief_prompt(recent_reports):
    if not recent_reports:
        return '{"health_score":0,"findings":[]}'
    lines = [f"  - {r.get('store_id')}: sales={r.get('sales')}, "
             f"inv={r.get('inventory_status')}, staff={r.get('staffing')}, "
             f"note={r.get('analysis')}" for r in recent_reports]
    body = chr(10).join(lines)
    return (
        f"Act as a Strategic Retail Intelligence Officer. Analyze the following fleet data:\n{body}\n\n"
        "Provide a comprehensive Strategic Intelligence Brief in JSON format:\n"
        "1. 'health_score': A score from 0-100 based on current performance vs. fleet norms.\n"
        "2. 'findings': A list of exactly 3 strategic findings. Each finding must have:\n"
        "   - 'title': A short, punchy headline.\n"
        "   - 'desc': A detailed explanation of the insight and the implied risk/opportunity.\n"
        "   - 'category': Must be one of ['Critical', 'Warning', 'Opportunity'].\n"
...
# Override with proper module if available
try:
    from brain.analytics import (
        analyze_store_status, calculate_7day_baseline,
        identify_red_zone_stores, generate_fleet_summary_prompt, calculate_fleet_kpis,
    )
except Exception as _e:
    import logging as _lg
    _lg.getLogger(__name__).warning("brain.analytics unavailable (%s) — using inline fallbacks", _e)
...
.tag-critical { color: var(--red); background: var(--red-dim); border-color: var(--red-border); }
.tag-warning { color: var(--amber); background: var(--amber-dim); border-color: var(--amber-border); }
.tag-opportunity { color: var(--green); background: var(--green-dim); border-color: var(--green-border); }
.finding-desc {
    font-size: clamp(11px,1.7vw,13px);
    color: var(--text-secondary);
    line-height: 1.5;
...
# ─── Data ───────────────────────────────────────────────────────────────────────
# All columns the dashboard expects — missing ones get a safe default
REQUIRED_COLS = {
    'store_id':         '',
    'sales':            0,
    'inventory_status': '—',
...
def load_data():
    ...
    # Ensure every expected column exists with a safe default
    ...
    # Fill nulls in text columns so .str.contains() never crashes
    for col in ['store_id','inventory_status','staffing','analysis']:
...
try:
    prompt      = generate_intelligence_brief_prompt(recent_df_raw.to_dict('records'))
    brief_raw = ai.client.chat.completions.create(
        model=ai.model,
        messages=[{"role": "user", "content": prompt}],
        response_format={"type": "json_object"}
    ).choices[0].message.content
    brief = json.loads(brief_raw)
    score = brief.get('health_score', '—')
    findings = brief.get('findings', [])

    findings_html = ""
    for f in findings:
        cat = f.get('category', 'Warning')
        tag_cls = f"tag-{'critical' if cat == 'Critical' else 'warning' if cat == 'Warning' else 'opportunity'}"
        findings_html += f'<div class="intel-finding"><div class="finding-header"><div class="finding-title">{f.get("title", "—")}</div><span class="finding-tag {tag_cls}">{cat}</span></div><div class="finding-desc">{f.get("desc", "—")}</div></div>'

    st.markdown(f"""
    <div class="intel-brief">
        <div class="ai-header">
            <div class="ai-title" style="font-size:1.2rem;">⚡ Strategic Intelligence Brief</div>
            <span class="ai-score">Fleet Health: {score}/100</span>
        </div>
        {findings_html}
    </div>""", unsafe_allow_html=True)
except Exception:
    st.markdown("""
    <div class="intel-brief">
        <div class="ai-header"><div class="ai-title">⚡ Strategic Intelligence Brief</div></div>
        <div class="finding-desc" style="color:var(--text-muted);">
            Unable to generate brief. Ensure <code class="ai-pill">GROQ_API_KEY</code> is configured.
        </div>
    </div>""", unsafe_allow_html=True)
...
with main_col:
    ...
    with tabs[1]:
        ...
        # Ensure all expected columns exist before slicing
        for _col, _def in [('inventory_status','—'),('staffing','—'),('analysis','—')]:
            if _col not in store_hist.columns:
                store_hist = store_hist.copy()
                store_hist[_col] = _def
...
if search_term:
    mask = (
        df['store_id'].astype(str).str.contains(search_term, case=False, na=False) |
        df['analysis'].astype(str).str.contains(search_term, case=False, na=False) |
        df['inventory_status'].astype(str).str.contains(search_term, case=False, na=False)
    )
    filtered = df[mask]
